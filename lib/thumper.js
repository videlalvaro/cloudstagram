    ...
    services.getRabbitMqConnection(function(conn) {
        if (consumers instanceof Array) {
            for(var i=0; i < consumers.length; i++) {
                anonConsumer(conn, consumers[i].exchange, consumers[i].routingKey, consumers[i].callback);
            }
        } else {
            anonConsumer(conn, consumers.exchange, consumers.routingKey, consumers.callback);
        }
    });
}

function anonConsumer(conn, exchangeName, routingKey, callback) {
    var exchange = {
        name: exchangeName,
